...
* 2. Aesthetics & UI
** Dynamic Font & Theme Presets
Instead of manually swapping fonts and syncing doom, use =M-x lumi/load-ui-preset= to swap predefined UI setups instantly!
...
Removes `evil-` prefixes in the which-key popup to make it much cleaner.
...
* 3. Org Mode & Workflows
** Settings & Core Properties
Basic setup for Org variables, directories, and auto-indentation behaviors.
...
** Org Modern
The `org-modern` package restyles standard Org-mode elements into modern, minimalistic variants. It hides emphasis markers and replaces ASCII symbols for lists, blocks, and stars with aesthetically pleasing unicode elements for a frictionless writing experience.
...
** SVG Tags
`svg-tag-mode` visually replaces textual labels such as `TODO`, `DONE`, and priority markers like `[#A]` with beautifully rendered, modern-looking SVG pill badges inside your Emacs buffer. It makes reviewing task lists much easier to parse.
...
** Symbol Prettifying
Prettify Symbols Mode visually replaces specific strings of text (like Org-mode metadata markers such as `#+title:`) with compact, intuitive Unicode icons without modifying the underlying plaintext. This dramatically cleans up the visual noise of Org property drawers and metadata.
...
** Org Source Blocks
Configures how Org-mode handles embedded code blocks (i.e., `org-src` blocks) by ensuring that Emacs preserves the natural indentation from your code when editing these blocks, preventing formatting modifications during exports.
...
** Org-habit & Org-heatmap & Org-journal
#+begin_src emacs-lisp :tangle no
...
** Citar & Noter
Citar is a citation management package. Org-noter is a tool for annotating PDFs and other documents with Org-mode.
...
** Auto-Completion (Corfu, Orderless, Hippie-Expand)
#+begin_src emacs-lisp :tangle yes
(use-package! corfu
  :config
...
** Codeforces Workflow
A custom workflow built for solving Codeforces (CF) programming puzzles natively in Emacs!
...
* 7. Miscellany (EPUBs, Scripts, Spells)
...
(use-package! nov
  ...
  (defun +nov-mode-setup ()
    ...
    (face-remap-add-relative 'variable-pitch :family "Alegreya" :height 1.1 :width 'semi-expanded)
...
(defun org-nov-open-new-window (path)
  ...
  (let* ((available-windows (delete (selected-window) (window-list)))
         (new-window (or (car available-windows) (split-window-sensibly) (split-window-right))))
