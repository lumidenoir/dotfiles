...
theme=$(printf '%s\n' "${all_themes[@]}" | rofi -dmenu -p "Select wallpaper theme: " -lines 10 -no-custom)
...
# Set some variables
wall_dir="${HOME}/dotfiles/wallpaper/${theme}"
cacheDir="${HOME}/.cache/$(whoami)/wallpaper/${theme}"
rofi_command="rofi -dmenu -theme ${HOME}/dotfiles/scripts/WallSelect.rasi -theme-str ${rofi_override}"
